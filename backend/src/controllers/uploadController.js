...
const imageSignatures = {
  ...
  "image/avif": buffer => buffer.length >= 16 && buffer.subarray(4, 8).toString() === "ftyp" && ["avif", "avis"].includes(buffer.subarray(8, 12).toString()),
};

const validateImageSignature = file => imageSignatures[file.mimetype]?.(file.buffer) === true;
...
exports.upload = multer({
  ...
  fileFilter: (req, file, callback) => {
    if (["image/jpeg", "image/png", "image/webp", "image/avif"].includes(file.mimetype)) return callback(null, true);
...
exports.uploadProductImage = async (req, res) => {
  ...
  try {
    ...
    if (!metadata.width || !metadata.height || metadata.width > MAX_DIMENSION || metadata.height > MAX_DIMENSION) {
      return res.status(400).json({ error: `Image dimensions must not exceed ${MAX_DIMENSION}x${MAX_DIMENSION}px` });
    }
    ...
    return res.status(400).json({ error: "The uploaded file is not a supported, valid image" });
  }
